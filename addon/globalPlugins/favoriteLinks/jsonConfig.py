...
from .varsConfig import getAddonConf
...
class JsonConfig:
	...
	def loadConfig(self):
		try:
			...
			log.debug(
				f"[{ourAddon.name}] JsonConfig loaded: index={self.indexJson}, primary='{self.firstJsonFile}', alternate='{self.altJsonFile}'",
			)

		except ValueError:
			log.error(
				f"[{ourAddon.name}] Invalid value for JSON index in configuration, using default: {self.defaultPath}",
			)
			self.browserPath = ""
			self.firstJsonFile = self.defaultPath
			self.altJsonFile = ""
	...
	def saveConfig(self):
		addonConf = getAddonConf()
		addonConf["browserPath"] = self.browserPath
		addonConf["path"] = self.firstJsonFile
		addonConf["altPath"] = self.altJsonFile
		addonConf["xx"] = str(self.indexJson)
		log.debug(
			f"[{ourAddon.name}] JsonConfig saved: index={self.indexJson}, primary='{self.firstJsonFile}', alternate='{self.altJsonFile}'",
		)
	...
	def setJsonPath(self, newPath, isFirst=True):
		...
		log.debug(
			f"[{ourAddon.name}] JsonConfig path set: {'primary' if isFirst else 'alternate'} to '{newPath}'",
		)
	...
	def getCurrentJsonPath(self):
		...
		if not currentPath:  # Fallback in case the selected path is unexpectedly empty
			log.warning(
				f"[{ourAddon.name}] Current JSON path is empty, falling back to default: {self.defaultPath}",
			)
	...
	def updateJsonFilePath(self, newPath):
		...
		if os.path.exists(oldActivePath) and not os.path.exists(newPath) and oldActivePath != newPath:
			try:
				...
				log.error(
					f"[{ourAddon.name}] Error renaming JSON file from '{oldActivePath}' to '{newPath}': {e}",
				)
		else:
			log.info(
				f"[{ourAddon.name}] JSON file path updated to '{newPath}'. No rename needed or file already exists/didn't exist.",
			)
